...
global_print_log = False


def print_log(*logs):
    global global_print_log
    if global_print_log:
        print(logs)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        # import pdb; pdb.set_trace()
        global global_print_log
        # print("before global_print_log:", global_print_log)
        B, T, C = x.size()  # batch size, sequence length, embedding dimensionality (n_embd)
        print_log("B, T, C:", B, T, C)
        ...
        if self.use_cache:
            ...
            if past_k is not None:
                print_log("past_k: ", past_k.shape)
                k = torch.cat((past_k, k), dim=1)
            if past_v is not None:
                print_log("past_v: ", past_v.shape)
                v = torch.cat((past_v, v), dim=1)
            self.past_key_values = (k, v)
            print_log("concat K:", k.shape)
            print_log("concat V:", v.shape)
            T = k.shape[1]
            q_len = q.shape[1]
        print_log("B, T, C, n_head, q_len:", B, T, C, self.n_head, q_len)

        k = k.view(B, T, self.n_head, C //
                   self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        q = q.view(B, q_len, self.n_head, C //
                   self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        ...
        if self.flash:
            ...
            print_log("att shape:", att.shape)
            if q_len != 1:
                att = att.masked_fill(
                    self.bias[:, :, :T, :T] == 0, float('-inf'))
            att = F.softmax(att, dim=-1)
            print_log("GPT input Attention att shape: ", att.shape)
            att = self.attn_dropout(att)
            print_log("GPT input Attention V shape: ", v.shape)
            y = att @ v  # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
            print_log("GPT input Attention y shape before: ", y.shape)
        # re-assemble all head outputs side by side
        y = y.transpose(1, 2).contiguous().view(B, q_len, C)

        # output projection
        y = self.resid_dropout(self.c_proj(y))
        print_log("GPT input Attention y shape: ", y.shape)
        global_print_log = False
        return y
...
class GPT(nn.Module):
    ...
    def forward(self, idx, past_len, targets=None):
        # import pdb; pdb.set_trace()
        device = idx.device
        b, t = idx.size()
        ...
        # print("GPT input x shape: ", x.shape)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            print("self.config.block_size: ", self.config.block_size)
            idx_cond = idx if idx.size(
                1) <= self.config.block_size else idx[:, -self.config.block_size:]
            print(f"============ {idx_cond} ========")
            # forward the model to get the logits for the index in the sequence
            # with record_function("## forward ##"):
            logits, _ = self(idx_cond, pre_length)
